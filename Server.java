...
this.port = port;
	...
	try 
	{
		...
		if(!keepGoing)
		    break;
		//so = new DataOutputStream(socket.getOutputStream());
		//si  = new DataInputStream(socket.getInputStream());
		//logIn();
...
/*    protected void stop() {
	...
	System.out.println("Prts: " + w[0] + "|" + w[1] + "|" + w[2]);
	boolean isPrivate = false;
	if(w[2].charAt(0)=='@') 
	    isPrivate=true;
	//System.out.println(w[2].charAt(0));
	
	// if private message, send message to mentioned username only
	if(isPrivate==true)
	{
	    String tocheck = subName(w[2]);
	    String self = w[0]; 
		//System.out.println("Name: " + tocheck);
	    message = w[0] + "(PVT)" + tocheck + w[1] + getMes(w[2]);
	    String messageLf = time + " " + message + "\n";
		//System.out.println("Mes: " + messageLf);
	    boolean found=false;
		...
		String chd = ct1.getUsername() + ":";
		    System.out.println("chd:" + "(" + chd + ") " + "self:" + "(" + self + ") ");
		if(check.equals(tocheck) || chd.equals(self))
		{
	...
	ClientThread(Socket socket) {
		...
		//si = new DataInputStream(socket.getInputStream());
		// read the username
		//broadcast("Waiting for password:");
		//while(true) {
		login = sInput.readUTF();
		String[] lp = login.split(" ");
		ps = lp[0];
		username = lp[1];
		...
		if(ps.equals(P1)) {
		    ...
		    //broadcast("NO ENTER!");
		    System.out.println("YOU SHELL NOT PASS!!! " + ps);
		    writeMsg("YOU SHELL NOT PASS!!!");
		    sInput.close();
		    sOutput.close();
		}
		//username = sInput.readUTF();
